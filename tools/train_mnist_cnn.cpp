...
int main()
{
    ...
    zerograd::Sequential cnn;
    cnn.add(std::make_shared<zerograd::Conv2d>(1, 8, 3, 3, 1, 1));
    cnn.add(std::make_shared<zerograd::ReLU>());
    cnn.add(std::make_shared<zerograd::MaxPool2d>(2, 2, 2, 0));
    cnn.add(std::make_shared<zerograd::Conv2d>(8, 16, 3, 3, 1, 1));
    cnn.add(std::make_shared<zerograd::ReLU>());
    cnn.add(std::make_shared<zerograd::MaxPool2d>(2, 2, 2, 0));
    cnn.add(std::make_shared<zerograd::Flatten>());
    cnn.add(std::make_shared<zerograd::Linear>(784, 128));
    cnn.add(std::make_shared<zerograd::ReLU>());
    cnn.add(std::make_shared<zerograd::Linear>(128, 10));

    std::size_t batch_size = 64;
    zerograd::DataLoader train_loader(train_data, batch_size);

    float lr = 1e-1;
    zerograd::Optimizer optimizer(cnn.parameters(), lr);
